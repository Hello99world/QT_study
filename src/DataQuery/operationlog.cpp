...
// 连接槽函数
void OperationLog::connectSignals()
{
    connect(btnQuery, &QPushButton::clicked, this, &OperationLog::onQueryClicked);
    connect(btnDelete, &QPushButton::clicked, this, &OperationLog::onDeleteClicked);
    connect(btnFirst, &QPushButton::clicked, this, &OperationLog::onFirstPageClicked);
    connect(btnPrev, &QPushButton::clicked, this, &OperationLog::onPrevPageClicked);
    connect(btnNext, &QPushButton::clicked, this, &OperationLog::onNextPageClicked);
    connect(btnLast, &QPushButton::clicked, this, &OperationLog::onLastPageClicked);
}

// 连接数据库
void OperationLog::initDatabase()
{
...
// 下拉框数据查询
void OperationLog::loadComboBoxData()
{
    QSqlDatabase db = DbHelper::getDatabase("iot.db");
    QSqlQuery query(db);
    // 控制器列表
    ...
    query.exec("SELECT DISTINCT DeviceName FROM nodelog WHERE DeviceName IS NOT NULL");
    while (query.next())
    {
    ...
    cmbNode->clear();
    cmbNode->addItem("全部");
    query.exec("SELECT DISTINCT NodeName FROM nodelog WHERE NodeName IS NOT NULL");
    while (query.next())
    {
...
// 构建查询SQL语句拼接
QString OperationLog::buildQuerySql(bool isCount)
{
    QString sql;
    // 插叙条件为空
    if (isCount)
    {
        sql = "SELECT COUNT(*) FROM nodelog WHERE 1=1";
    }
    else
    {
        sql = "SELECT LogID, PositionID, DeviceName, NodeName, NodeValue, NodeSign, "
              "SaveTime, LogMark FROM nodelog WHERE 1=1";
    }
    // 控制器
    ...
    {
        sql += " AND SaveTime >= :startTime AND SaveTime <= :endTime";
    }
    // 位置编号
...
// 构建查询参数
void OperationLog::bindQueryParams(QSqlQuery &query)
{
    // 时间
    if (chkTime->isChecked())
    {
...
void OperationLog::pageQuery()
{
    ...
    if (query.exec())
    {
        ...
        model->setHeaderData(4, Qt::Horizontal, "浓度值");
        model->setHeaderData(5, Qt::Horizontal, "气体符号");
        model->setHeaderData(6, Qt::Horizontal, "保存时间");
...
// 更新分页按钮的状态
void OperationLog::updatePageStatus()
{
...
// 提取显示分页信息
void OperationLog::showPageInfo()
{
...
// 查询按钮点击槽函数
void OperationLog::onQueryClicked()
{
    currentPage = 1;
    pageQuery();
}
...
// 删除符合条件的记录槽函数
void OperationLog::onDeleteClicked()
{
    int count = getCount();
    if (count == 0)
    {
...
// 首页槽函数
void OperationLog::onFirstPageClicked()
{
    currentPage = 1;
    pageQuery();
}
...
// 上一页槽函数
void OperationLog::onPrevPageClicked()
{
    if (currentPage > 1)
    {
        currentPage--;
...
// 下一页槽函数
void OperationLog::onNextPageClicked()
{
...
// 尾页槽函数
void OperationLog::onLastPageClicked()
{
